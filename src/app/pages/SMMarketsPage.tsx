...
export function SMMarketsPage() {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-[#FFFFFF] flex items-center gap-2">🛒 SM Markets</h1>
        <p className="text-gray-500 dark:text-[#9E9E9E] text-sm mt-1">Compare prices across SM branches</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-[#1E1E1E] text-gray-700 dark:text-[#E0E0E0] outline-none focus:border-green-400 text-sm"
        />
        <select
          value={branch}
          onChange={(e) => setBranch(e.target.value)}
          className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-[#1E1E1E] text-gray-700 dark:text-[#E0E0E0] outline-none text-sm min-w-0 sm:min-w-56"
        >
      ...
      <div className="flex items-center justify-between text-xs text-gray-400">
        ...
        <select value={sort} onChange={(e) => setSort(e.target.value)}
          className="px-3 py-1 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-[#1E1E1E] text-gray-700 dark:text-[#E0E0E0] outline-none">
        ...
        {categories.map((cat) => (
          <button key={cat} onClick={() => setCategory(cat)}
            className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
              category === cat ? "bg-green-600 text-white" : "bg-white dark:bg-[#1E1E1E] text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 hover:bg-green-50"
            }`}>{cat}</button>
        ))}
      </div>
      ...
      {sortedFiltered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {sortedFiltered.map((item) => (
            <div key={item.id} className="bg-white dark:bg-[#1E1E1E] rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-[#2D2D2D] hover:shadow-md transition-all">
              {item.aiTip && (
              ...
              <p className="font-semibold text-gray-800 dark:text-[#FFFFFF] text-sm mb-1">{item.name}</p>
              <p className="text-xs text-gray-400 mb-2">Pack: {item.pack}</p>
              <div className="flex items-end justify-between mb-3">
                <div>
                  <p className="text-gray-400 text-xs">Pack price</p>
                  <p className="font-bold text-gray-700 dark:text-[#E0E0E0]">₱{item.packPrice}</p>
                </div>
