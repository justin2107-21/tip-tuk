  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-sm font-medium group ${
      isActive
        ...
        : "text-gray-600 dark:text-gray-300 hover:bg-green-50 dark:hover:bg-green-900/30 hover:text-green-800 dark:hover:text-green-300"
    }`;

  return (
    <aside className={`${mobile ? "w-full" : "w-56 fixed left-0 top-16 bottom-0 overflow-y-auto"} bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col py-4 px-3`}>
      <div className="mb-1">
      ...
      <div className="mt-auto px-3 py-4">
        <div className="rounded-xl p-3 text-xs text-gray-500 dark:text-gray-400"
          ...
          <p className="mt-1 text-gray-400">Updated: {new Date().toLocaleDateString("en-PH")}</p>
        </div>
      </div>
    </aside>
  );
